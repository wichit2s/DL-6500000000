...
nc = 3  # Number of channels (rgb) in the training images.
...
def show_real_vs_fake(dataloader, image_list, device):
    real_batch = next(iter(dataloader))
    ...
    images = np.transpose(vutils.make_grid(real_batch[0].to(device)[:64], padding=5, normalize=True).cpu())
    plt.imshow(images, (1,2,0))
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        show_errors_plot(G_losses, D_losses)
        show_real_vs_fake(dataloader, image_list, device)
